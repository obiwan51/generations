...
export interface PlayerData {
  ...
  holding: number | null;
  holdingData: any;
  backpack: number | null;      // Container worn on back (e.g., basket)
  ...
  generation: number;           // Generation number (Eve = 1, children = mother + 1)
  inBoat: boolean;              // Whether player is currently in a boat
  lastMsg?: string;
  lastMsgTime?: number;
}
...
export class PlayerManager {
  ...
  private holdingBaby: Map<string, string> = new Map();
  // Track who is being held (babyId -> holderId)
  private heldBy: Map<string, string> = new Map();
  
  /** Callback when a player dies */
  public onPlayerDeath?: (playerId: string, reason: string, stats: PlayerData) => void;
  
  /** Callback when a baby is picked up or put down */
  public onBabyPickedUp?: (holderId: string, babyId: string) => void;
  public onBabyPutDown?: (holderId: string, babyId: string) => void;
  ...
  createPlayer(
    socketId: string,
    motherId: string | null,
    config: { maxHunger: number; spawnEveAge: number; maxAge: number },
    forcedGender?: 'male' | 'female',
    forcedName?: string
  ): { player: PlayerData; motherUpdate?: PlayerData; droppedItem?: { x: number; y: number; type: number; data: any } } {
    ...
    const identity = new IdentityComponent(
      ...
      motherName,
      gender,
      isEve ? 1 : motherGeneration + 1  // Generation
    );
    ...
    let motherUpdate: PlayerData | undefined;
    let droppedItem: { x: number; y: number; type: number; data: any } | undefined;
  ...
  movePlayer(
  ...
  ): { player: PlayerData; heldBaby?: PlayerData } | null {
    const entity = this.players.get(socketId);
    if (!entity) return null;
    ...
    let heldBaby: PlayerData | undefined;
    const heldBabyId = this.holdingBaby.get(socketId);
    if (heldBabyId) {
  ...
  pickUpBaby(holderId: string, babyId: string): boolean {
    ...
    if (!holderInv || holderInv.holding !== null) return false;
    if (!holderAge || holderAge.age < CONSTANTS.BABY_MAX_AGE) return false;
    
    // Baby must be young enough and not already held
    if (!babyAge || babyAge.age >= CONSTANTS.BABY_MAX_AGE) return false;
    if (this.heldBy.has(babyId)) return false;
    ...
    if (!holderPos || !babyPos) return false;
    ...
    if (holderTileX !== babyTileX || holderTileY !== babyTileY) return false;
  ...
  takeFromBackpack(socketId: string): number | null {
    ...
    const inventory = entity.getComponent(InventoryComponent);
    if (!inventory || !inventory.backpackData?.inventory?.length) return null;
