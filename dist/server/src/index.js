...
io.on('connection', (socket) => {
    ...
    function birthNewPlayer() {
        ...
        const fertileMothers = playersArr.filter(p => p.gender === 'female' &&
            p.age >= 16 && p.age < 60 &&
    ...
    socket.on('eat', () => {
        ...
        if (heldFoodVal > 0) {
            ...
            if (containerDef && playerData.holdingData?.inventory?.length > 0) {
                const idx = playerData.holdingData.inventory.findIndex((t) => getFoodValue(t) > 0);
                if (idx !== -1) {
    ...
    socket.on('pickUp', () => {
        ...
        if (!p.holding) {
            const playersOnTile = Object.values(engine.players).filter(other => other.id !== socket.id &&
    ...
    socket.on('takeFromBackpack', () => {
        const p = engine.players[socket.id];
        if (!p || !p.backpack || p.holding)
            return; // Need backpack and empty hands
    ...
    socket.on('use', () => {
        ...
        function findNearestLand(centerX, centerY) {
        ...
        if ((isItemContainer(toolDef) || isItemContainer(targetDef)) && toolObj !== null && targetObj !== null) {
            ...
            const basket = isTargetBasket
                ? { type: targetObj, data: targetData, isGround: true, def: targetDef }
            ...
            if (canFit && basket.def && basket.data.inventory.length < (basket.def.capacity || 3)) {
                ...
                if (basket.isGround) {
                    world.setObject(tx, ty, basket.type, basket.data);
                    io.emit('worldUpdate', { x: tx, y: ty, type: basket.type, data: basket.data });
                }
                else {
                    engine.updatePlayerHolding(socket.id, basket.type, basket.data);
                }
                const current = engine.getPlayerHoldingData(socket.id);
                io.emit('playerStatUpdate', { id: socket.id, holding: current?.holding ?? null, holdingData: current?.holdingData ?? null });
                return;
        ...
        if (recipe) {
            ...
            const handleToolDurability = () => {
                if (toolHasUses && toolObj !== null) {
                    ...
                    const currentUses = toolData?.usesRemaining ?? toolDef.uses;
                    const newUses = currentUses - 1;
                    if (newUses <= 0) {
                        // Tool is broken
                        engine.updatePlayerHolding(socket.id, null, null);
                        io.emit('playerStatUpdate', { id: socket.id, holding: null, holdingData: null });
                        io.to(socket.id).emit('textMessage', { text: `Your ${toolDef.name} broke!` });
                    }
                    else {
                        // Tool still has uses
