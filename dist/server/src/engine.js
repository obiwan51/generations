...
class GameEngine {
    ...
    setupHungerCallbacks() {
        // Provide age lookup for baby hunger rate
        this.hungerSystem.getPlayerAge = (componentId) => {
        ...
        this.hungerSystem.onHungerUpdate = (componentId, hunger) => {
    ...
    setupAnimalCallbacks() {
        ...
        this.animalSystem.getPlayersInRange = (x, y, range) => {
            ...
            for (const playerId of this.playerManager.getPlayerIds()) {
                ...
                if (!data)
                    continue;
                const dist = Math.sqrt(Math.pow(x - data.x / CONSTANTS.TILE_SIZE, 2) +
    ...
    handleProjectileHit(hit) {
        ...
        if (data.hp === undefined)
            data.hp = animalDef.hp;
        const projComp = hit.projectile.getComponent(ProjectileComponent);
        const dmg = projComp?.damage ?? 5;
        data.hp -= dmg;
        if (data.hp <= 0) {
            const recipe = this.recipes.find((r) => r.tool === projComp?.type && r.target === hit.targetType);
            if (recipe) {
    ...
    reloadRegistry() {
        const animals = registry.get("animals");
        const resources = registry.get("resources");
        const items = registry.get("items");
        this.animalRegistry = {};
        for (const key in animals)
            this.animalRegistry[animals[key].id] = animals[key];
        this.resourceRegistry = {};
        for (const key in resources)
            this.resourceRegistry[resources[key].id] = resources[key];
        this.itemRegistry = {};
        for (const key in items)
            this.itemRegistry[items[key].id] = items[key];
        const getId = (key) => {
        ...
        this.recipes = registry.get("recipes").map((r) => ({
            id: r.id,
            name: r.name,
     ...
     */
    registerPlantedCrop(x, y, objectType) {
        const itemDef = this.itemRegistry[objectType];
        if (itemDef && 'growsInto' in itemDef && 'growthTicks' in itemDef) {
            this.growthSystem.createComponent(x, y, objectType, itemDef.growsInto, itemDef.growthTicks || 3);
        }
    }
    /**
     ...
     */
    addPlayer(socketId, motherId, forcedGender, forcedName) {
    ...
    handleShoot(socketId, angle) {
        ...
        this.projectileSystem.createProjectile(socketId, data.x, data.y, finalAngle, projType, maxDist, weaponDef.weaponDamage || 5);
    }
    /**
    ...
    getStatistics() {
        ...
        this.statisticsManager.updateWorldStats(CONSTANTS.MAP_SIZE, world.objects, this.animalRegistry, this.resourceRegistry);
