...
export class InputSystem {
    ...
    // Bounce-to-use state
    bounceState = null;
    constructor(canvas, socket, audio) {
        this.canvas = canvas;
        this.socket = socket;
        this.audio = audio;
        this.setupListeners();
    ...
    setPlayerHoldingGetter(getter) {
        this.getPlayerHolding = getter;
    }
    setPlayerDataGetter(getter) {
        this.getPlayerData = getter;
    }
    setWorldDataGetter(getter) {
        this.getWorldData = getter;
    }
    setPlayersDataGetter(getter) {
        this.getPlayersData = getter;
    }
    setMyIdGetter(getter) {
        this.getMyId = getter;
    }
    ...
    getItemIdByKey(key) {
        if (!this.registry)
            return null;
        const item = this.registry.items[key];
        return item?.id ?? null;
    }
    ...
    setupListeners() {
        ...
        this.canvas.addEventListener('mousemove', (e) => {
            if (!this.callbacks.onShowObjectActions)
                return;
        ...
        if (clickedObject) {
            if (isPlayerCell && clickedObject === playerObject) {
                ...
                if (holding) {
                    ...
                    if (holdingMeta && holdingMeta.isEdible) {
     ...
     */
    getBabyAtPosition(worldX, worldY) {
     ...
     */
    getObjectCategory(id) {
        if (!this.registry)
            return null;
        if (Object.values(this.registry.items).find((i) => i.id === id)) {
            return 'item';
        }
     ...
     */
    hasBareHandsRecipe(targetId) {
        if (!this.recipes || this.recipes.length === 0)
            return false;
        // Bare hands recipes have tool: null
        return this.recipes.some(recipe => recipe.tool === null && recipe.target === targetId);
    }
    /**
