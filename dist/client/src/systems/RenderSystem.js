...
export class RenderSystem {
    ...
    loadRegistryImages(registryData) {
        for (const key in registryData.animals) {
            const animal = registryData.animals[key];
            const animalAssets = animal.assets;
        ...
        for (const key in registryData.resources) {
            const res = registryData.resources[key];
            if (res.asset) {
        ...
        for (const key in registryData.items) {
            const item = registryData.items[key];
            if (item.asset) {
    ...
    getObjectMetadata(id) {
        ...
        if (res)
            return { ...res, category: 'resource' };
        const ani = Object.values(this.registry.animals).find(a => a.id === id);
        if (ani)
            return { ...ani, category: 'animal' };
        const item = Object.values(this.registry.items).find(i => i.id === id);
        if (item)
    ...
    getObjectImage(type) {
        ...
        if (meta.category === 'resource' || meta.category === 'tool' || meta.category === 'material' || meta.category === 'structure' || meta.category === 'food' || meta.category === 'weapon' || meta.category === 'ammo' || meta.category === 'container' || meta.category === 'carcass' || meta.category === 'seed' || meta.category === 'crop') {
            return this.images[`/assets/${meta.asset}`];
        }
        else if (meta.category === 'animal') {
            return this.images[`/assets/${meta.assets.alive}`];
        }
        return null;
    }
    draw(world, players, projectiles, myId, season) {
    ...
    collectDrawables(world, players, offsetX, offsetY, TILE_SIZE, myId) {
        const drawables = [];
        ...
        for (const key in world.objects) {
            ...
            if (screenX + TILE_SIZE * 2 < 0 || screenX > this.canvas.width ||
                screenY + TILE_SIZE * 2 < 0 || screenY > this.canvas.height)
                continue;
    ...
    drawTileTexture(screenX, screenY, TILE_SIZE, tileX, tileY, biome, colors) {
        const ctx = this.ctx;
    ...
    drawObject(obj, screenX, screenY, TILE_SIZE, data) {
        const img = this.getObjectImage(obj);
        const meta = this.getObjectMetadata(obj);
        const isReady = img instanceof HTMLCanvasElement ||
            (img instanceof HTMLImageElement && img.complete && img.naturalWidth > 0);
        if (img && isReady) {
            ...
            if (meta) {
                if (meta.isLarge) {
                    ...
                    h = TILE_SIZE * scale;
                    w = h * imgAspect;
                    yOffset = meta.yOffset || TILE_SIZE;
            ...
            if (data?.hp !== undefined && meta?.category === 'animal') {
                this.drawHealthBar(screenX, screenY - yOffset - 5, TILE_SIZE, data.hp, meta.hp || 100);
            }
            if (data?.inventory?.length > 0) {
                data.inventory.forEach((itemType, index) => {
                    ...
                    if (subImg && subIsReady)
                        this.ctx.drawImage(subImg, screenX + 5 + (index * 15), screenY + 10, 20, 20);
                });
            }
        }
        else {
    ...
    drawHealthBar(x, y, tileSize, hp, maxHp) {
    ...
    drawProjectiles(projectiles, offsetX, offsetY) {
